...
def read_site_timezone(base_path: str) -> str | None:
    """Read the ical timezone from hugo.toml config."""
    hugo_toml = Path(base_path) / "hugo.toml"
    if not hugo_toml.exists():
        return None
    try:
        config = tomllib.loads(hugo_toml.read_text(encoding="utf-8"))
        # Check params.ical.timezone first (explicit ical config)
        tz = config.get("params", {}).get("ical", {}).get("timezone")
        if tz:
            return tz
...
class ICalValidator:
    ...
    def validate_html_output(
        ...
        """Validate HTML <time datetime> attributes against frontmatter timestamps."""
        if not self.site_timezone:
            return
        ...
        if not time_elements:
            return  # No <time> elements to validate

        tz = ZoneInfo(self.site_timezone)

        # Collect expected datetime values from frontmatter
        expected_pairs: list[tuple[str, Any]] = []
        if "startDate" in frontmatter:
            expected_pairs.append(("startDate", frontmatter["startDate"]))
        if "endDate" in frontmatter:
            expected_pairs.append(("endDate", frontmatter["endDate"]))

        # Extract all datetime attribute values from <time> elements
        html_datetimes: list[str] = []
        for time_el in time_elements:
            dt_attr = time_el.get("datetime", "")
            if dt_attr:
                html_datetimes.append(dt_attr)

        for field_name, fm_value in expected_pairs:
            # Parse frontmatter value (PyYAML may have already converted to datetime)
            fm_str = str(fm_value)
            fm_dt = datetime.fromisoformat(fm_str)

            if fm_dt.tzinfo is None:
                # Bare timestamp — apply site timezone
                fm_dt = fm_dt.replace(tzinfo=tz)

            # The HTML datetime should represent the same instant in the site timezone
            fm_in_site_tz = fm_dt.astimezone(tz)

            # Check if any <time datetime> matches this expected value
            found_match = False
            for html_dt_str in html_datetimes:
                try:
                    html_dt = datetime.fromisoformat(html_dt_str)
                    # Compare as absolute instants (both converted to UTC)
                    if html_dt.astimezone(timezone.utc) == fm_in_site_tz.astimezone(
                        timezone.utc
                    ):
                        found_match = True
                        break
            ...
            if not found_match:
                self.log_error(
                    f"HTML output missing correct <time datetime> for {field_name}: "
                    f"expected {fm_in_site_tz.isoformat()} "
                    f"(from frontmatter '{fm_str}' via {self.site_timezone}), "
                    f"found: {html_datetimes}",
                    ics_file=html_file,
                )
    ...
    def validate_ics_file(self, ics_file: str, md_file: str | None = None) -> bool:
        ...
        if md_file and Path(md_file).exists():
            ...
            if frontmatter:
                ...
                # Validate HTML output (only for calendar.ics to avoid duplicate checks)
                if Path(ics_file).name == CALENDAR_FILENAME:
                    html_file = Path(ics_file).parent / "index.html"
                    if html_file.exists():
                        self.validate_html_output(str(html_file), frontmatter)
...
def main() -> None:
    ...
    site_timezone = read_site_timezone(args.base_path)
    validator = ICalValidator(site_timezone=site_timezone)
